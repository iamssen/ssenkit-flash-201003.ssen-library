package ssen.core.display.graphics 
{
	 ...
	 */
	public class TextTrash 
	{
		private static var _bounds : Shape = new Shape();
		private var _ef : ElementFormat;
		...
		public function TextTrash(box : Rectangle = null, padding : Padding = null, elementFormat : ElementFormat = null, textColor : uint = 0x000000, align : String = "left", valign : String = "middle", text : String = "")
		{
		...
		public function draw(graphics : Graphics, shift : Boolean = false, shiftSpace : int = 0) : void		
		{
			...
			if (_boxChanged || _shapeChanged) {
				...
				var rect : Rectangle = _line.getBounds(_bounds);
				_mat.identity();
				_mat.ty = rect.height;
				_bitmap = new BitmapData(rect.width, rect.height - rect.y, true, 0x00ffffff);
				_bank.push(_bitmap);
				_lineBox.width = _bitmap.width;
				_lineBox.height = _bitmap.height;
				_bitmap.draw(_line, _mat);
			}
			_boxChanged = false;
			_shapeChanged = false;
		...
		{
			_element.text = text;
			_element.elementFormat = _ef;
			_block.content = _element;
			if (_line) _line = null;
			_line = _block.createTextLine(null, width);
		}
		public function get fontDescription() : FontDescription
		{
			_shapeChanged = true;
			return _ef.fontDescription;
		}
		public function set fontDescription(fontDescription : FontDescription) : void
		{
			_ef.fontDescription = fontDescription;
			_shapeChanged = true;
		}
		...
		public function set elementFormat(elementFormat : ElementFormat) : void
		{
			_ef = elementFormat;
			_shapeChanged = true;
		}
